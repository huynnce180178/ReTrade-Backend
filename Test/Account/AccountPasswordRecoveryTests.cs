...
namespace Test.AccountTests
{
    public class AccountPasswordRecoveryTests
    {
        ...
        public AccountPasswordRecoveryTests()
        {
        ...
        [Fact]
        public async Task PasswordRecoveryAsync_ReturnsSuccess_WhenEmailExists()
        {
            ...
            var result = await _service.PasswordRecoveryAsync(email);

            result.Should().Be("Password reset successful. Please check your email for your new password.");
            _emailService.Verify(x => x.SendEmailAsync(email, It.IsAny<string>(), It.IsAny<string>()), Times.Once);
            _accountRepo.Verify(x => x.UpdateAsync(It.IsAny<Account>()), Times.Once);
        }

        [Fact]
        public async Task PasswordRecoveryAsync_ReturnsError_WhenEmailDoesNotExist()
        {
            ...
            result.Should().Be("No account is associated with this email address.");
