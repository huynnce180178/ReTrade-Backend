...
namespace Test.AccountTests
{
    public class AccountRoleAssignTests
    {
        ...
        [Fact]
        public async Task UTCD01_Assign_role_successfully_with_valid_account_ID_and_role_ID()
        {
            var accountId = "A1";
            var roleId = 1;
            ...
            _roleRepository.Setup(x => x.AssignRoleAsync(accountId, roleId)).ReturnsAsync(true);

            var result = await _service.AssignRoleAsync(accountId, roleId);

            result.Should().BeTrue();
            _accountService.Verify(x => x.GetByIdAsync(accountId), Times.Once);
            _roleRepository.Verify(x => x.AssignRoleAsync(accountId, roleId), Times.Once);
        }

        [Fact]
        public async Task UTCD02_Assign_role_with_a_null_account_ID_returns_false()
        {
            var result = await _service.AssignRoleAsync(null!, 1);

            result.Should().BeFalse();
            _accountService.Verify(x => x.GetByIdAsync(It.IsAny<string>()), Times.Never);
            _roleRepository.Verify(x => x.AssignRoleAsync(It.IsAny<string>(), It.IsAny<int>()), Times.Never);
        }

        [Fact]
        public async Task UTCD03_Assign_role_with_an_invalid_account_ID_returns_false()
        {
            var accountId = "invalid";
            _accountService.Setup(x => x.GetByIdAsync(accountId)).ReturnsAsync((Account?)null);

            var result = await _service.AssignRoleAsync(accountId, 1);
        ...
        [Fact]
        public async Task UTCD04_Assign_role_with_a_null_role_ID_returns_false()
        {
            var accountId = "A1";
            ...
            var result = await _service.AssignRoleAsync(accountId, default);
        ...
        [Fact]
        public async Task UTCD05_Assign_role_with_an_invalid_role_ID_returns_false()
        {
            var accountId = "A1";
            var roleId = 999;
            _accountService.Setup(x => x.GetByIdAsync(accountId)).ReturnsAsync(new Account { AccountId = accountId });
